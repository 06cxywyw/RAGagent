# 小林coding - OS-CPU执行程序
> 代码写了那么多，你知道a = 1 + 2这条代码是怎么被 CPU 执行的吗？
---
## 2.1 CPU 是如何执行程序的？
代码写了那么多，你知道a = 1 + 2这条代码是怎么被 CPU 执行的吗？
...
那么，想要 CPU 操作 4G 大的内存，那么就需要 32 条地址总线，因为2 ^ 32 = 4G。
...
另外，32 位 CPU 最大只能操作 4GB 内存，就算你装了 8 GB 内存条，也没用。而 64 位 CPU 寻址范围则很大，理论最大的寻址空间为2^64。