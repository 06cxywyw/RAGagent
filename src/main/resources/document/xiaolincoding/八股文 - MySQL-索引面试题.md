...
TIP
光看不练，面试怎么能行？
如果你想针对MySQL进展专项的模拟面试，可以用小林和朋友合作打造的「[牛面AI面试](https://www.niumianoffer.com/?ref=tswekk4d)」来检测MySQL的掌握情况：
- 点击访问：[MySQL专项面试](https://www.niumianoffer.com/?ref=tswekk4d)
...
TIP
读者问题：如果MySQL表有联合索引（a，b），为什么MySQL的 select * from table where a>2 and b=1不走b索引，而 select * from table where a>=2 and b=1 能走b索引？
回答：
执行where a>=2 and b=1 会被拆成两个sql：
- where a>2 and b=1 ，这一条 b 不能走索引
- where a=2 and b=1 ，这一条 ab都能走索引
为什么 where a>2 and b=1 ，这一条 b 不能走索引？
举个例子，a>1 的情况下，有这些记录：
- a=2，b=8
- a=2，b=9
- a=3，b=2
- a=3，b=3
a>1 的查询，并不是按 a =2 、a =3这样去扫描索引的，而是 a 走了索引找到了第一个大于1的记录，也就是a=2，b=8开始，接着就会根据叶子节点间的链表往后扫描记录。这时候 b 并不是有序的（b=8，b=9，b=2，b=3）无法利用二分查找的特性快速查找到b。
...
- 第二个原因：NULL 值是一个没意义的值，但是它会占用物理空间，所以会带来的存储空间的问题，因为 InnoDB 存储记录的时候，如果表中存在允许为 NULL 的字段，那么[行格式](https://xiaolincoding.com/mysql/base/row_format.html#innodb-%E8%A1%8C%E6%A0%BC%E5%BC%8F%E6%9C%89%E5%93%AA%E4%BA%9B)中至少会用 1 字节空间存储 NULL 值列表，如下图的紫色部分：
---
### 防止索引失效
