...
> 来源: https://javaguide.cn/cs-basics/network/http-vs-https.html

# HTTP vs HTTPS：区别在哪里、HTTPS 为什么更安全（应用层）
[Guide](https://javaguide.cn/article/)计算机基础计算机网络约 3182 字大约 11 分钟HTTP 能传输网页内容，但默认是明文传输。请求和响应如果在网络中被监听、篡改或冒充，HTTP 本身没有足够的保护能力。

HTTPS 不是一个全新的应用层协议，而是在 HTTP 和 TCP 之间加入 TLS/SSL，用加密、身份认证和完整性校验来保护通信过程。

这篇文章主要回答几个问题：

- HTTP 和 HTTPS 的核心区别是什么？
- HTTPS 如何防止窃听、篡改和冒充？
- SSL/TLS 握手大致做了哪些事情？
- 为什么使用 HTTPS 后，证书、混合内容和性能优化仍然需要关注？
...
![](https://javaguide.cn/assets/public-key-cryptography-BQGPLr2_.png)
...
![单向函数](https://javaguide.cn/assets/OWF-BWBna_be.png)
...
![](https://javaguide.cn/assets/symmetric-encryption-01EOzq7r.png)
...
![](https://javaguide.cn/assets/attack1-ftBWLwVN.png)
...
![](https://javaguide.cn/assets/digital-signature--6DaTXPY.png)
...
![](https://javaguide.cn/assets/public-key-transmission-CNWLGx6C.png)
