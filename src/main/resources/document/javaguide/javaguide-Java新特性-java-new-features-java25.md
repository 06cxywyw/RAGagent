...
> 来源: https://javaguide.cn/java/new-features/java25.html

# Java 25 新特性概览
[Guide](https://javaguide.cn/article/)JavaJava新特性约 2610 字大约 9 分钟JDK 25 于 2025 年 9 月 16 日 发布，这是一个非常重要的版本，里程碑式。
...
---
### JEP 506: 作用域值
作用域值（Scoped Values）可以在线程内和线程间共享不可变的数据，优于线程局部变量ThreadLocal，尤其是在使用大量虚拟线程时。
...
这个改进极大地简化了编写简单 Java 程序的步骤，允许将类和主方法写在同一个没有顶级public class的文件中，并允许main方法成为一个非静态的实例方法。
...
```
这是为了降低 Java 的学习门槛和提升编写小型程序、脚本的效率而迈出的一大步。初学者不再需要理解public static void main(String[] args)这一长串复杂的声明。对于快速原型验证和脚本编写，这也使得 Java 成为一个更有吸引力的选择。
...
- JDK 25 之后仅需-XX:+UseCompactObjectHeaders即可启用。

---
### JEP 521: 分代 Shenandoah GC
Shenandoah GC 在 JDK12 中成为正式可生产使用的 GC，默认关闭，通过-XX:+UseShenandoahGC启用。
...
- JDK 25 之后仅需-XX:+UseShenandoahGC -XX:ShenandoahGCMode=generational即可启用。
