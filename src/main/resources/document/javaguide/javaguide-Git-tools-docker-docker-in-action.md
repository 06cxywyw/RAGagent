...
> 来源: https://javaguide.cn/tools/docker/docker-in-action.html

# Docker实战
[Guide](https://javaguide.cn/article/)开发工具Docker约 5069 字大约 17 分钟
---
## Docker 介绍
...
- Docker 是世界领先的软件容器平台，基于Go 语言进行开发实现。
...
```
然而下载镜像的过程是非常慢的，所以我们需要配置一下镜像源加速下载，访问阿里云官网，点击控制台：
...
若想查看 Docker 中当前拥有哪些镜像，则可以使用docker images命令。
...
```
在/opt/apps目录下创建了一个test.html文件，那么容器内的webapps目录是否会有该文件呢？进入容器的终端：
