...
> 来源: https://javaguide.cn/ai/interview-questions/agent-interview-questions.html

# AI Agent 面试题总结
[Guide](https://javaguide.cn/article/)AIAgent面试AI AgentAI面试约 3700 字大约 12 分钟AI Agent 面试最容易出现两种极端：一种是把 Agent 讲得像“全自动数字员工”，什么都能自己规划、自己执行；另一种是把 Agent 讲得像“几个 Prompt 串起来”，完全看不出和普通工作流有什么区别。
...
---
## Agent 基础
参考文章：[《AI Agent 核心概念：Agent Loop、Context Engineering、Tools 注册》](https://javaguide.cn/ai/agent/agent-basis.html)
...
---
## Agent Memory
参考文章：[《AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制》](https://javaguide.cn/ai/agent/agent-memory.html)
...
参考文章：[《大模型提示词工程实践指南》](https://javaguide.cn/ai/agent/prompt-engineering.html)、[《上下文工程实战指南：让 Agent 少犯蠢的工程方法论》](https://javaguide.cn/ai/agent/context-engineering.html)
...
---
## MCP 与 Agent Skills
参考文章：[《深入理解 MCP 协议：一次开发，多处复用》](https://javaguide.cn/ai/agent/mcp.html)、[《Agent Skills 是什么？和 Prompt、MCP 到底差在哪？》](https://javaguide.cn/ai/agent/skills.html)
...
---
## Harness Engineering
参考文章：[《一文搞懂 Harness Engineering：六层架构、上下文管理与一线团队实战》](https://javaguide.cn/ai/agent/harness-engineering.html)
...
参考文章：[《AI 工作流中的 Workflow、Graph 与 Loop：从概念到实现》](https://javaguide.cn/ai/agent/workflow-graph-loop.html)
...
- 先定义任务类型：是问答、检索、工具调用、多步骤任务，还是长周期任务。
- 再选择编排方式：纯 Agent、Workflow、Agentic Workflow 或 Multi-Agent。
- 接着讲核心组件：Context、Memory、Tools、MCP、Skills、State。
- 然后讲安全和稳定性：权限、校验、超时、重试、审计、成本控制。
- 最后讲评测：任务完成率、工具调用准确率、轨迹质量和失败样本回放。
