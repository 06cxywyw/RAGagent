...
> 来源: https://javaguide.cn/ai/agent/context-engineering.html

# 上下文工程实战指南：让 Agent 少犯蠢的工程方法论
[Guide](https://javaguide.cn/article/)AI 应用开发约 6097 字大约 20 分钟同样的模型，同样的 Agent 框架，为什么有的人跑起来很稳，你的一跑就开始迷路？
...
- 静态规则、动态信息、Token 预算、按需加载怎么落地
- Compaction、结构化笔记、Sub-agent 怎么解决长任务上下文问题
...
- 挂载create_return_order和check_inventory工具
...
这是静态规则，比如.cursorrules、.claude/rules、AGENTS.md这类文件。里面一般会放角色设定、目标、约束、执行流、输出格式。
...
```
这些规则可以固化到.cursorrules或AGENTS.md文件里。
...
最常见的坑，是做一个“大而全”的工具。比如manage_database，里面同时包含建表、查数据、删数据、备份、导出五个能力。Agent 选择工具时会犹豫，填参数时也容易被一堆无关字段干扰。
...
Claude Code 就是很典型的例子。它分析大型代码库时，不会把所有文件都塞进上下文，而是先通过目录结构、文件名、搜索命令定位目标，再用head、tail、grep这类方式逐步读取。

Agent 像人一样靠文件名和目录结构理解信息位置，靠文件大小和时间戳判断优先级，而不是上来就把全部内容吞进去。

这里有个很容易被忽略的点：元数据本身也是信息。

tests/test_utils.py和src/core_logic/test_utils.py语义就不一样。光看路径，Agent 就能判断它们大概率服务于不同目的。
...
Claude Code 也是这个思路：CLAUDE.md文件可以预加载，但具体文件内容靠 Agent 运行时探索。
...
Structured Note-taking 是另一种长任务处理方式。让 Agent 把关键进展写到外部文件里，比如NOTES.md。上下文重置后，再读取这些笔记继续工作。

这和人类工程师写 to-do list、技术备忘很像。Claude Code 在长任务里会自动维护 to-do list。自定义 Agent 也可以在项目根目录维护NOTES.md，里面记录当前进度、已知问题、下一步计划。
