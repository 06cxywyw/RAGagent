...
> 来源: https://javaguide.cn/ai/agent/prompt-engineering.html

# 大模型提示词工程实践指南
[Guide](https://javaguide.cn/article/)AI 应用开发约 5159 字大约 17 分钟刚学 Prompt 的时候，很多人都会犯一个毛病：恨不得把所有背景、要求、限制都塞进去。
...
- Prompt 的四要素框架：指令、背景、输入、输出怎么拆
- 六种常用提示技巧：角色扮演、思维链、少样本、任务分解、结构化输出、XML 标签
- 复杂场景怎么处理：长文本、多步骤任务、格式不稳定
- 企业级安全实践：Prompt Injection 防御和输出消毒
- Prompt 在 Agent 系统里的位置，和 Context Engineering 的关系

前置知识：本文默认你已经理解 Token、上下文窗口、Temperature、Top-p 等 LLM 底层概念。如果还不熟，可以先看[《万字拆解 LLM 运行机制：Token、上下文与采样参数》](https://javaguide.cn/ai/llm-basis/llm-operation-mechanism.html)。
