...
> 来源: https://javaguide.cn/ai/interview-questions/rag-interview-questions.html

# RAG 面试题总结
[Guide](https://javaguide.cn/article/)AIRAG面试向量数据库AI面试约 4094 字大约 14 分钟RAG 是 AI 应用开发里最容易被低估的模块。
...
- 先理解 RAG 解决什么问题，以及它和微调、长上下文、传统搜索的区别。
- 再理解 Embedding、相似度、ANN 索引和向量数据库选型。
- 接着理解文档处理、Chunk 策略、元数据和权限过滤。
- 然后掌握 Hybrid Search、Query Rewrite、Rerank、上下文压缩等优化手段。
- 最后补上 GraphRAG、知识库更新和评测闭环。
...
---
## RAG 基础
参考文章：[《万字详解 RAG 基础概念》](https://javaguide.cn/ai/rag/rag-basis.html)
...
---
## 向量数据库与索引
参考文章：[《万字详解 RAG 向量索引算法和向量数据库》](https://javaguide.cn/ai/rag/rag-vector-store.html)
...
---
## 文档处理与 Chunk 策略
参考文章：[《RAG 文档处理与切分策略：从解析、清洗、Chunking 到多模态内容处理》](https://javaguide.cn/ai/rag/rag-document-processing.html)
...
---
## RAG 检索优化
参考文章：[《万字详解 RAG 优化：从召回、重排到上下文工程的系统调优》](https://javaguide.cn/ai/rag/rag-optimization.html)
...
---
## GraphRAG
参考文章：[《万字详解 GraphRAG：为什么只靠向量检索撑不起复杂知识问答》](https://javaguide.cn/ai/rag/graphrag.html)
...
---
## 知识库更新与评测
参考文章：[《RAG 知识库文档如何更新：增量更新、版本控制、去重与全量重建》](https://javaguide.cn/ai/rag/rag-knowledge-update.html)、[《AI 应用评测体系：从 Golden Set 构建到线上灰度闭环》](https://javaguide.cn/ai/llm-basis/llm-evaluation.html)
...
- 问题理解：用户问题是否口语化、缩写、多意图、需要多跳推理。
- 文档处理：原始文档是否解析正确，Chunk 是否保留语义和元数据。
- 召回阶段：正确证据是否进入候选池，召回池是否足够大。
- 排序阶段：正确证据是否排在前面，是否需要 Rerank。
- 上下文阶段：证据是否被截断、重复、污染，是否存在 Lost in the Middle。
- 生成阶段：模型是否忠实基于证据回答，是否需要引用和拒答策略。
- 评测阶段：是否有稳定样本集，是否能复现问题。
