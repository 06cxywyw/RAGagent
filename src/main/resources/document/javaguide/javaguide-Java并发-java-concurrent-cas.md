...
> 来源: https://javaguide.cn/java/concurrent/cas.html

# CAS 详解
[Guide](https://javaguide.cn/article/)JavaJava并发约 1941 字大约 6 分钟乐观锁和悲观锁的介绍以及乐观锁常见实现方式可以阅读笔者写的这篇文章：[乐观锁和悲观锁详解](https://javaguide.cn/java/concurrent/optimistic-lock-and-pessimistic-lock.html)。
...
Atomic 类依赖于 CAS 乐观锁来保证其方法的原子性，而不需要使用传统的锁机制（如synchronized块或ReentrantLock）。
...
---
## 总结
在 Java 中，CAS 通过Unsafe类中的native方法实现，这些方法调用底层的硬件指令来完成原子操作。由于其实现依赖于 C++ 内联汇编和 JNI 调用，因此 CAS 的具体实现与操作系统以及 CPU 密切相关。
